...
type Mode = 'large' | 'medium' | 'compact' | 'svg' | 'bmp' | 'svg-uri' | 'bmp-uri';
// {
...
// }
type ErrorCorrectionLevel = typeof QrCode.ErrorCorrectionLevel;
type UserOptions = {
  text: string;
  version?: number;
...
// options - the configuration object for the QR Code (optional).
//   Options include errorCorrectionLevel (0-3),
//   optimizeEcc (boolean flag, default true, to maximize the error-correction level within the chosen output size),
//   minVersion/maxVersion (1-40),
//   maskPattern (0-7).
//   Hints for the rendering stage are invert (boolean flag to invert the code, not as widely supported),
//   and quiet (the size, in modules, of the quiet area around the code).
type RenderOptions = {
  moduleSize?: number;
  white?: boolean;
  moduleRound?: number;
...
//   svg / svg-uri: moduleSize the unit dimensions of each module,
//   white (boolean) output the non-set
//   modules (otherwise will be transparent background),
//   moduleRound proportion of how rounded the modules are,
//   finderRound to hide the standard finder modules and instead output a shape with the specified roundness,
//   alignmentRound to hide the standard alignment modules and instead output a shape with the specified roundness.

//   bmp / bmp-uri: scale for the size of a module,
//   alpha (boolean) to use a transparent background,
//   width/height can set a specific image size (rather than scaling the matrix dimensions).

// --- Bit Buffer Writing ---
class BitBuffer {
  buffer: Uint8Array;
  bitOffset: number;
  constructor(public bitCapacity: number) {
  ...
  read(bitPosition: number): number {
    const value: number = this.buffer[bitPosition >> 3] & (1 << (7 - (bitPosition & 7))) ? 1 : 0;
    return value;
  }
}
...
class Segment {
  ...
  public mode: typeof SegmentAlphanumeric | typeof SegmentEightBit | typeof SegmentNumeric;
  // ECI Assignment Numbers
  //static ECI_UTF8 = 26; // "\000026" UTF8 - ISO/IEC 10646 UTF-8 encoding

  constructor(public text: string) {
    this.text = text;
    for (const mode of Object.values(Segment.MODES)) {
...
class ReedSolomon {
  ...
  static Remainder(
    ...
    for (let i = 0; i < dataLen; i++) {
      const factor = data[dataOffset + i] ^ result[resultOffset + 0];
...
class Matrix {
  ...
  static calculateDimension(version: number) {
    return 17 + 4 * version; // V1=21x21; V40=177x177
  }

  static calculateMask(maskPattern: number, j: number, i: number) {
    switch (maskPattern) {
      case 0:
  ...
  static alignmentCoordinates(version: number) {
  ...
  setModule(x: number, y: number, value: number, identity?: string): void {
  ...
  identifyModule(x: number, y: number) {
  ...
  drawFinder(ox: number, oy: number) {
    for (let y = -Math.floor(Matrix.FINDER_SIZE / 2) - 1; y <= Math.floor(Matrix.FINDER_SIZE / 2) + 1; y++) {
      for (let x = -Math.floor(Matrix.FINDER_SIZE / 2) - 1; x <= Math.floor(Matrix.FINDER_SIZE / 2) + 1; x++) {
        let value =
          (Math.abs(x) > Math.abs(y) ? Math.abs(x) : Math.abs(y)) & 1 ? Matrix.MODULE_DARK : Matrix.MODULE_LIGHT;
  ...
  drawAlignment(ox: number, oy: number) {
    for (let y = -Matrix.ALIGNMENT_RADIUS; y <= Matrix.ALIGNMENT_RADIUS; y++) {
      for (let x = -Matrix.ALIGNMENT_RADIUS; x <= Matrix.ALIGNMENT_RADIUS; x++) {
        const value =
          1 - ((Math.abs(x) > Math.abs(y) ? Math.abs(x) : Math.abs(y)) & 1) ? Matrix.MODULE_DARK : Matrix.MODULE_LIGHT;
  ...
  populateFunctionPatterns() {
    ...
    for (const h of alignmentCoords) {
      for (const v of alignmentCoords) {
  ...
  cursorWrite(buffer: BitBuffer, sourceBit: number, countBits: number) {
    let index = sourceBit;
    for (let countWritten = 0; countWritten < countBits; countWritten++) {
      const bit = buffer.read(index);
  ...
  drawVersionInfo(value: number | null) {
    const id = 'Ve';
  ...
  applyMaskPattern(maskPattern: number) {
  ...
  evaluatePenalty() {
    ...
    for (let swapAxis = 0; swapAxis <= 1; swapAxis++) {
      ...
      for (let y = 0; y < this.dimension; y++) {
        ...
        let runLength = 0;
        for (let x = 0; x < this.dimension; x++) {
          const bit = this.getModule(swapAxis ? y : x, swapAxis ? x : y);
          // Run extended
          ...
          if (bit != lastBit || x >= this.dimension - 1) {
            ...
            if (lastBit >= 0) {
              // Feature 1
              if (runLength >= 5) {
                // or should this be strictly greater-than?
              ...
              if (runsCount >= 5) {
                ...
                const v = runs[(runsCount + 1) % 5];
                if (
                  runs[runsCount % 5] == 3 * v &&
                  v == runs[(runsCount + 2) % 5] &&
                  v == runs[(runsCount + 3) % 5] &&
                  v == runs[(runsCount + 4) % 5]
                ) {
    ...
    // Feature 4: Dark module percentage: 50 +|- (5*k) to 50 +|- (5*(k+1)), penalty points: N4 * k
    {
      ...
      for (let y = 0; y < this.dimension; y++) {
        for (let x = 0; x < this.dimension; x++) {
          const bit = this.getModule(x, y);
          if (bit == Matrix.MODULE_DARK) darkCount++;
        }
      }
      // Deviation from 50%
      const percentage = (100 * darkCount + (this.dimension * this.dimension) / 2) / (this.dimension * this.dimension);
      const deviation = Math.abs(percentage - 50);
      const rating = Math.floor(deviation / 5);
      const penalty = scoreN4 * rating;
...
class QrCode {
  ...
  static dimension(version: number) {
    return 17 + 4 * version;
  }
  ...
  static totalDataModules(version: number) {
    return (
      (16 * version + 128) * version +
      64 -
      (version < 2 ? 0 : (25 * (Math.floor(version / 7) + 2) - 10) * (Math.floor(version / 7) + 2) - 55) -
      (version < 7 ? 0 : 36)
    );
  }

  // Calculate the total number of data bits available in the codewords (cooked: after ecc and remainder)
  static dataCapacity(version: number, errorCorrectionLevel: number) {
    const capacityCodewords = Math.floor(QrCode.totalDataModules(version) / 8);
    const eccTotalCodewords =
      QrCode.eccBlockCodewords(version, errorCorrectionLevel) * QrCode.eccBlockCount(version, errorCorrectionLevel);
  ...
  static eccBlockCount(version: number, errorCorrectionLevel: number) {
  ...
  static eccBlockCodewords(version: number, errorCorrectionLevel: number) {
    const eccBlockCodewordsLookup = [
      ...
      [
        0, 7, 10, 15, 20, 26, 18, 20, 24, 30, 18, 20, 24, 26, 30, 22, 24, 28, 30, 28, 28, 28, 28, 30, 30, 26, 28, 30,
  ...
  static calculateVersionInfo(version: number) {
  ...
  static calculateFormatInfo(errorCorrectionLevel: number, maskPattern: number) {
  ...
  static measureSegments(segments: Segment[], version: number) {
    let total = 0;
  ...
  static doSegmentsFit(segments: Segment[], version: number, errorCorrectionLevel: number) {
  ...
  static tryToImproveErrorCorrectionLevel(
    segments: Segment[],
    version: number,
    currentErrorCorrectionLevel: ErrorCorrectionLevel[keyof ErrorCorrectionLevel]
  ) {
  ...
  static writeData(scratchBuffer: BitBuffer, version: number, segments: Segment[]) {
  ...
  static writePadding(scratchBuffer: BitBuffer, version: number, errorCorrectionLevel: number) {
  ...
  static calculateEccAndFillMatrix(
    scratchBuffer: BitBuffer,
    version: number,
    errorCorrectionLevel: number,
    matrix: Matrix
  ) {
    ...
    console.assert(
      8 * eccOffset === scratchBuffer.bitOffset,
      `Expected current bit position ${scratchBuffer.bitOffset} to match ECC offset *8 ${8 * eccOffset}`
    );
    ...
    for (let block = 0; block < eccBlockCount; block++) {
      ...
      const dataLen = dataLenShort + (block < countShortBlocks ? 0 : 1);
      // Calculate this block's ECC
      const eccDest = eccOffset + block * eccCodewords;
      ReedSolomon.Remainder(
  ...
  //
  static findOptimalMaskPattern(matrix: Matrix) {
    let lowestPenalty = -1;
    let bestMaskPattern = undefined;
  ...
  static generate(text: string, userOptions?: UserOptions) {
    // Generation options
    const options: UserOptions = Object.assign(
      {
    ...
    let maskPattern: number | undefined = options.maskPattern;
    if (maskPattern === null || maskPattern === undefined) {
      const patern = QrCode.findOptimalMaskPattern(matrix);
      if (patern === undefined) throw 'Unable to find optimal mask pattern';
      maskPattern = patern;
    }
  ...
  static render(mode: Mode, matrix: Matrix, renderOptions?: RenderOptions) {
    const renderers = {
      debug: renderDebug,
...
function BitmapGenerate(data: Array<[number, number, number, number]>, width: number, height: number, alpha = false) {
  ...
  if (bmpHeaderSize >= bmpHeaderSizeByVersion.BITMAPV2INFOHEADER) {
    // (14+52=66) BITMAPV2INFOHEADER (+RGB BI_BITFIELDS)
  ...
  if (bmpHeaderSize >= bmpHeaderSizeByVersion.BITMAPV3INFOHEADER) {
    // (14+56=70) BITMAPV3INFOHEADER (+A BI_BITFIELDS)
  ...
  if (bmpHeaderSize >= bmpHeaderSizeByVersion.BITMAPV4HEADER) {
    // (14+108=122) BITMAPV4HEADER (color space and gamma correction)
  ...
  if (bmpHeaderSize >= bmpHeaderSizeByVersion.BITMAPV5HEADER) {
    // (14+124=138) BITMAPV5HEADER (ICC color profile)
...
function renderDebug(matrix: Matrix, options?: RenderOptions) {
  ...
  for (let y = -matrix.quiet; y < matrix.dimension + matrix.quiet; y++) {
    ...
    for (let x = -matrix.quiet; x < matrix.dimension + matrix.quiet; x++) {
      let part: Segment | string | undefined = matrix.identifyModule(x, y);
...
function renderTextLarge(matrix: Matrix, options?: RenderOptions) {
  options = Object.assign(
    {
      segments: ['  ', '██'],
      sep: '\n'
  ...
  for (let y = -matrix.quiet; y < matrix.dimension + matrix.quiet; y++) {
    ...
    for (let x = -matrix.quiet; x < matrix.dimension + matrix.quiet; x++) {
      ...
      const chars =
        options.segments!.length >= 3 && bit && !matrix.identifyModule(x, y)
          ? options.segments![2]
...
function renderTextMedium(matrix: Matrix, options?: RenderOptions) {
  options = Object.assign(
    {
      segments: [' ', '▀', '▄', '█'],
      sep: '\n'
...
function renderTextCompact(matrix: Matrix, options?: RenderOptions) {
  options = Object.assign(
    {
      segments: [' ', '▘', '▝', '▀', '▖', '▌', '▞', '▛', '▗', '▚', '▐', '▜', '▄', '▙', '▟', '█'],
      sep: '\n'
  ...
  for (let y = -matrix.quiet; y < matrix.dimension + matrix.quiet; y += 2) {
    ...
    for (let x = -matrix.quiet; x < matrix.dimension + matrix.quiet; x += 2) {
      ...
      value |= ((x + 1 < matrix.dimension ? matrix.getModule(x + 1, y) : 0) ? !matrix.invert : matrix.invert)
        ? 0x02
        : 0x00;
      value |= ((y + 1 < matrix.dimension ? matrix.getModule(x, y + 1) : 0) ? !matrix.invert : matrix.invert)
        ? 0x04
        : 0x00;
      value |= (
        (y + 1 < matrix.dimension && x + 1 < matrix.dimension ? matrix.getModule(x + 1, y + 1) : 0)
          ? !matrix.invert
          : matrix.invert
      )
        ? 0x08
...
function renderSvg(matrix: Matrix, defaultOptions?: RenderOptions) {
  ...
  // viewport-fill=\"white\"
  lines.push(
    `<svg xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" fill="currentColor" viewBox="${vbTopLeft} ${vbTopLeft} ${vbWidthHeight} ${vbWidthHeight}" shape-rendering="crispEdges">`
  );
  ...
  lines.push(
    `<rect id="b" x="${-options.moduleSize! / 2}" y="${-options.moduleSize! / 2}" width="${
      options.moduleSize
    }" height="${options.moduleSize}" rx="${0.5 * (options.moduleRound || 0) * options.moduleSize!}" />`
  );
  ...
  if (options.finderRound != null) {
    ...
    lines.push(
      `<g id="fc"><rect x="-3" y="-3" width="6" height="6" rx="${
        3.0 * options.finderRound
      }" stroke="currentColor" stroke-width="1" fill="none" /><rect x="-1.5" y="-1.5" width="3" height="3" rx="${
        1.5 * options.finderRound
      }" /></g>`
    );
    lines.push(
      `<g id="fc"><rect x="-3" y="-3" width="6" height="6" rx="${
  ...
  if (options.alignmentRound != null) {
    ...
    lines.push(
      `<g id="ac"><rect x="-2" y="-2" width="4" height="4" rx="${
        2.0 * options.alignmentRound
      }" stroke="currentColor" stroke-width="1" fill="none" /><rect x="-0.5" y="-0.5" width="1" height="1" rx="${
        0.5 * options.alignmentRound
      }" /></g>`
    );
  } else {
  ...
  for (let y = 0; y < matrix.dimension; y++) {
    for (let x = 0; x < matrix.dimension; x++) {
      ...
      let bit: number | boolean | null = matrix.getModule(x, y);
...
function renderSvgUri(matrix: Matrix, options?: RenderOptions) {
  return 'data:image/svg+xml,' + encodeURIComponent(renderSvg(matrix, options));
}

function renderBmp(matrix: Matrix, defaultOptions?: RenderOptions) {
  const options: RenderOptions = Object.assign(
    {
      scale: 8,
      alpha: false,
  ...
  if (!options.height || !options.width) throw new Error('Invalid width or height');
  ...
  for (let y = 0; y < options.height; y++) {
    ...
    for (let x = 0; x < options.width; x++) {
      ...
      const bit = matrix.getModule(mx, my);
      let color;
      if (matrix.invert) {
...
function renderBmpUri(matrix: Matrix, options?: RenderOptions) {
...
export const createQrSVG = (value: string) => {
  ...
  return QrCode.render('svg', matrix, {
    alignmentRound: 0.5,
    finderRound: 0.5,
    moduleRound: 1,
    alpha: true
